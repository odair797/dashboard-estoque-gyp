...
Gera Analise_Estoque_GYP.xlsx — modelo padrão:
  - Produto Acabado : Linha + colunas padrão
  - Matéria Prima / Embalagem / Granel / Apoio : colunas padrão
  - Em Processo : Família + colunas padrão (registros com Setor Doca/Packing)
Colunas padrão: SKU, Descrição, Quantidade, Lote Indústria, Local, Setor,
                Data de Vencimento, Dias a Vencer, Meses a Vencer, Criticidade.
Regra: registros cujo Setor seja Doca ou Packing saem das abas de família
       e vão para a aba Em Processo (com a Família de origem na 1a coluna).
"""
import sys
try:
...
# Setores "em processo" (indisponiveis): Doca / Packing
PROC_RE = re.compile(r'DOCA|PACKING', re.IGNORECASE)
def em_processo(setor):
    return bool(PROC_RE.search(str(setor or '')))
...
CRIT_FILL = {
    ...
    # variantes sem acento que vem dos CSVs (Criticidade do processar_estoque)
    'VENCIDO ':      (RED_CRIT, WHITE),
}
...
def write_row(ws, row, headers, valores):
    """Escreve uma linha de dados; formata colunas pelo NOME do cabecalho."""
    ...
    if 'Descrição' in idx:
        ws.cell(row=row, column=idx['Descrição']).alignment = Alignment(horizontal='left', vertical='center')
    # Formatos numéricos
    if 'Quantidade' in idx:
        ws.cell(row=row, column=idx['Quantidade']).number_format = '#,##0.000'
    if 'Dias a Vencer' in idx:
        ws.cell(row=row, column=idx['Dias a Vencer']).number_format = '0'
    if 'Meses a Vencer' in idx:
        ws.cell(row=row, column=idx['Meses a Vencer']).number_format = '0.0'
    # Criticidade colorida
    if 'Criticidade' in idx:
        cc = idx['Criticidade']
        crit_val = valores[cc-1]
...
def rec_padrao(r):
    """Valores padrão (sem Linha/Família) a partir de uma linha de CSV."""
    return [
...
processo_rows = []   # acumula registros Doca/Packing -> aba Em Processo
...
for _, r in pa[pa_proc_mask].iterrows():
    processo_rows.append(['Produto Acabado'] + rec_padrao(r))

ws = wb.create_sheet('📦 Produto Acabado')
title_row(ws, 1, len(HDRS_PA), f'PRODUTO ACABADO  |  {len(pa_disp)} registros  |  {DATA_STR}')
...
for linha, grupo in pa_disp.groupby('Linha', sort=True):
    ...
    for _, r in grupo.iterrows():
        write_row(ws, row, HDRS_PA, [''] + rec_padrao(r))  # Linha em branco no detalhe
        row += 1
...
# ═══ ABAS POR FAMÍLIA ═══════════════════════════════════════════
familias = [
    ('MP',  '🧪 Matéria Prima', 'MATÉRIA PRIMA', 'Matéria Prima'),
    ('EMB', '📦 Embalagem',     'EMBALAGEM',     'Embalagem'),
    ('GRA', '🛢️ Granel',         'GRANEL',        'Granel'),
    ('APO', '🔧 Apoio',         'APOIO',         'Apoio'),
]
...
for short, sheet_name, fam_upper, fam_nome in familias:
    ...
    for _, r in sub[proc_mask].iterrows():
        processo_rows.append([fam_nome] + rec_padrao(r))

    wsf = wb.create_sheet(sheet_name)
    title_row(wsf, 1, len(HDRS_FAM), f'{fam_upper}  |  {len(disp)} registros  |  {DATA_STR}')
    header_row(wsf, 2, HDRS_FAM)
    row = 3
    for _, r in disp.iterrows():
        write_row(wsf, row, HDRS_FAM, rec_padrao(r))
        row += 1
    for i,w in enumerate(WID_FAM,1):
        wsf.column_dimensions[get_column_letter(i)].width = w
    wsf.freeze_panes = 'A3'

# ═══ ABA EM PROCESSO (Doca / Packing) ═══════════════════════════
wsp = wb.create_sheet('⏳ Em Processo')
title_row(wsp, 1, len(HDRS_PROC), f'EM PROCESSO (DOCA / PACKING)  |  {len(processo_rows)} registros  |  {DATA_STR}')
header_row(wsp, 2, HDRS_PROC)
row = 3
# ordenar por Família e depois por SKU
processo_rows.sort(key=lambda v: (str(v[0]), str(v[1])))
for valores in processo_rows:
    write_row(wsp, row, HDRS_PROC, valores)
    # destacar a coluna Família
    cf = wsp.cell(row=row, column=1)
    cf.font = Font(name='Calibri', size=9, bold=True, color=WHITE)
    cf.fill = PatternFill('solid', fgColor=PROC_BG)
    cf.alignment = Alignment(horizontal='center', vertical='center')
    row += 1
for i,w in enumerate(WID_PROC,1):
...
print(f'  Em Processo (Doca/Packing): {len(processo_rows)} registros')
